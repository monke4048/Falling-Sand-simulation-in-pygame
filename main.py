...
def Clacultae(maxHe,maxWi,xpos,ypos,static,blocks):
        if ypos < maxHe and [xpos,ypos + 1,1,0,1] not in blocks and [xpos,ypos + 1,2,0,1] not in blocks and [xpos,ypos + 1,1,0,2] not in blocks and [xpos,ypos + 1,2,0,2] not in blocks and [xpos,ypos + 1,1,0,3] not in blocks and [xpos,ypos + 1,2,0,3] not in blocks:
                        ypos += 1
                        static = 1
        elif ypos < maxHe and xpos < maxWi and [xpos + 1,ypos + 1,1,0,1] not in blocks and [xpos + 1,ypos + 1,2,0,1] not in blocks and [xpos + 1,ypos ,2,0,1] not in blocks and [xpos + 1,ypos + 1,1,0,2] not in blocks and [xpos + 1,ypos + 1,2,0,2] not in blocks and [xpos + 1,ypos ,2,0,2] not in blocks and [xpos + 1,ypos + 1,1,0,3] not in blocks and [xpos + 1,ypos + 1,2,0,3] not in blocks and [xpos + 1,ypos ,2,0,3] not in blocks:
                        xpos += 1
                        ypos += 1
                        static = 1
        elif ypos < maxHe and xpos > 0 and [xpos - 1,ypos + 1,1,0,1] not in blocks and [xpos - 1,ypos + 1,2,0,1] not in blocks and [xpos - 1,ypos,2,0,1] not in blocks and [xpos - 1,ypos + 1,1,0,2] not in blocks and [xpos - 1,ypos + 1,2,0,2] not in blocks and [xpos - 1,ypos,2,0,2] not in blocks and [xpos - 1,ypos + 1,1,0,3] not in blocks and [xpos - 1,ypos + 1,2,0,3] not in blocks and [xpos - 1,ypos,2,0,3] not in blocks:
                xpos -= 1
                ypos += 1
                static = 1
        else:
...
while True:
    ...
    if mKEY[0]:
        ...
        if [m[0] //gridsize,m[1]//gridsize,2,0,1] not in blocks and [m[0] //gridsize,m[1]//gridsize,2,0,2] not in blocks and [m[0] //gridsize,m[1]//gridsize,2,0,3] not in blocks:
               if [m[0] //gridsize,m[1]//gridsize,1,1,1] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,0,1] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,1,2] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,0,2] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,1,3] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,0,3] not in blocks:
                        blocks.append([m[0] //gridsize,m[1]//gridsize,1,1,1])
    if mKEY[1]:
        m = pygame.mouse.get_pos()
        
        if [m[0] //gridsize,m[1]//gridsize,2,0,1] not in blocks and [m[0] //gridsize,m[1]//gridsize,2,0,2] not in blocks and [m[0] //gridsize,m[1]//gridsize,2,0,3] not in blocks:
               if [m[0] //gridsize,m[1]//gridsize,1,1,1] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,0,1] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,1,2] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,0,2] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,1,3] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,0,3] not in blocks:
                        blocks.append([m[0] //gridsize,m[1]//gridsize,1,1,2])
    if mKEY[2]:
        m = pygame.mouse.get_pos()
        
        if [m[0] //gridsize,m[1]//gridsize,2,0,1] not in blocks and [m[0] //gridsize,m[1]//gridsize,2,0,2] not in blocks and [m[0] //gridsize,m[1]//gridsize,2,0,3] not in blocks:
               if [m[0] //gridsize,m[1]//gridsize,1,1,1] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,0,1] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,1,2] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,0,2] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,1,3] not in blocks and [m[0] //gridsize,m[1]//gridsize,1,0,3] not in blocks:
                        blocks.append([m[0] //gridsize,m[1]//gridsize,1,1,3])
    ...
    for i in blocks:
        if i[2] == 1:
                if dead:
                        ...
                        if i[4] == 1:
                                pygame.draw.rect(win,(181,49,32),(i[0] * gridsize,i[1]*gridsize,gridsize,gridsize))
                        if i[4] == 2:
                                pygame.draw.rect(win,(234,158,34),(i[0] * gridsize,i[1]*gridsize,gridsize,gridsize))
                        if i[4] == 3:
